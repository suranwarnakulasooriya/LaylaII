...
class Bot_Info:
    def __init__(self,prefix,token):
        ...
        self.rate = 5 # rate at which users level up relative to messages sent, the rate is linear: new nxp needed to level up = rate * next level
    ...
    async def on_message(self,message):
        if not message.author.bot:
            if message.author.id not in U:
                U[message.author.id] = User(message.author.id,self.obj)
...
with open('users.txt') as f:
    usersraw = f.readlines()

usersstr = []
for user in usersraw:
    usersstr.append(user.split())
